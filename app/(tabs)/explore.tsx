...
// ========================================================
// IMPORT SIMULASI ROLE (ASUMSI DARI AuthProvider)
// ========================================================
const useAuth = () => ({
    // Ganti 'admin' dengan 'user' atau 'guest' untuk menguji tampilan non-admin
    role: 'user' as 'admin' | 'user' | 'guest', // SIMULASI ROLE USER (non-admin)
});
// ========================================================
...
// CARD_WIDTH = Lebar layar dikurangi padding kiri/kanan dan spacing.
...
export default function TabTwoScreen() {
    ...
    const { role } = useAuth(); // Ambil peran pengguna
                            ...
                            <View key={item.id} style={styles.featuredCard}>
                                <View style={styles.featuredCardInner}>
                                    ...
                                    {role !== 'admin' ? (
                                        ...
                                        <View style={[styles.featuredReviewBtn, styles.adminReviewBtn]}>
                                            <IconSymbol size={18} color="#0f172a" name="lock.fill" />
                                            <Text style={styles.adminReviewText}>Akses Admin</Text>
                                        </View>
                                    )}
                                </View>
                            </View>
                    ...
                    {regularHotels.map((item, index) => (
                        ...
                        >
                            <View style={styles.regularCard}>
                                ...
                                {/* Right: Review Button - Disesuaikan */}
                                {role !== 'admin' ? (
                                    <TouchableOpacity
                                    ...
                                    <View style={[styles.regularReviewBtn, styles.adminReviewBtnRegular]}>
                                        {/* Tampilan Admin: Tombol Edit? Atau hanya label Terkunci */}
                                        <View style={styles.adminReviewInner}>
                                            <IconSymbol size={18} color="#0f172a" name="gearshape.fill" />
                                            <Text style={styles.adminReviewTextRegular}>Admin</Text>
                                        </View>
                                    </View>
                                )}
                            </View>
    ...
    return (
        <ThemedView style={styles.wrapper}>
            ...
            {/* FAB Button (untuk Admin, jika ada) */}
            {role === 'admin' && (
                <Animated.View
                    style={[
                        styles.fabContainer,
                ...
                >
                    <TouchableOpacity
                        style={styles.fab}
                        onPress={() => router.push('/forminputlocation')} // Mengarahkan ke input lokasi/hotel
                        activeOpacity={0.85}
                    >
...
const styles = StyleSheet.create({
    ...
    carouselContent: {
        paddingRight: CARD_SPACING, // Sesuaikan padding kanan agar kartu terakhir tidak menempel
    },
    featuredCard: {
        width: CARD_WIDTH,
    ...
    // GAYA KHUSUS ADMIN (FEATURED)
    adminReviewBtn: {
        backgroundColor: '#e2e8f0', // Latar belakang abu-abu muda
        borderColor: '#cbd5e1',
        shadowColor: 'transparent',
        opacity: 0.8,
    },
    adminReviewText: {
        color: '#0f172a', // Teks lebih gelap
        fontSize: 14,
        fontWeight: '900',
    },
    ...
    // GAYA KHUSUS ADMIN (REGULAR)
    adminReviewBtnRegular: {
        justifyContent: 'center',
        backgroundColor: 'transparent',
    },
    adminReviewInner: {
        backgroundColor: '#fcd34d', // Kuning/emas untuk Admin
        paddingHorizontal: 10,
        paddingVertical: 10,
        borderRadius: 12,
        alignItems: 'center',
